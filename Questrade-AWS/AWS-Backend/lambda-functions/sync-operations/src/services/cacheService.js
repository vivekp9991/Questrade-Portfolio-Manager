...
class CacheService {
  ...
  async cacheAccounts(personName, accounts) {
    const cacheKey = `accounts-${personName}`;
    // 1-day TTL (was 7): account changes (closures/openings) should be picked up quickly.
    // Full/scheduled syncs also force a fresh fetch via getAccountsWithCache({forceFresh}).
    return await this.setCache(cacheKey, accounts, 1);
  }
