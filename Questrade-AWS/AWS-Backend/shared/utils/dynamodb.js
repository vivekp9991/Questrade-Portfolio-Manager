...
/**
 * Conditional update — used for distributed locking and optimistic concurrency.
 * Performs an UpdateCommand guarded by a ConditionExpression.
 *
 * @param {string} tableName
 * @param {object} key
 * @param {object} updates              field -> value to SET
 * @param {string} conditionExpression  e.g. 'attribute_not_exists(lockUntil) OR lockUntil < :now'
 * @param {object} [extraValues]        extra ExpressionAttributeValues referenced by the condition (e.g. { ':now': Date.now() })
 * @param {object} [extraNames]         extra ExpressionAttributeNames (rarely needed)
 * @returns {Promise<{success:boolean, attributes?:object}>}
 *          success=false when the condition failed (ConditionalCheckFailedException); other errors throw.
 */
async function conditionalUpdate(tableName, key, updates, conditionExpression, extraValues = {}, extraNames = {}) {
  const updateExpressions = [];
  const expressionAttributeNames = { ...extraNames };
  const expressionAttributeValues = { ...extraValues };

  Object.keys(updates).forEach((field, index) => {
    const placeholder = `#cu${index}`;
    const valuePlaceholder = `:cu${index}`;
    updateExpressions.push(`${placeholder} = ${valuePlaceholder}`);
    expressionAttributeNames[placeholder] = field;
    expressionAttributeValues[valuePlaceholder] = updates[field];
  });
