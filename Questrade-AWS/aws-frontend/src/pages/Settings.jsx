...
import QuestradeApiTest from '../components/QuestradeApiTest';
...
export default function Settings() {
        ...
        personsArray.map(async (person) => {
          try {
            ...
            const lastSync = personSyncs.length > 0
              ? (personSyncs[0].syncTimestamp || personSyncs[0].timestamp || personSyncs[0].startTime || personSyncs[0].createdAt)
              : null;
  ...
  const handleLoadToken = async () => {
    ...
    try {
      ...
      const mappedData = {
        ...
        refreshTokenIsActive: data.refreshToken?.isActive,
  ...
  const handleUpdateToken = async () => {
    ...
    try {
      setLoading(true);
      await settingsApi.updatePersonToken(selectedPerson(), newRefreshToken().trim());
  ...
  const sortedAndPaginatedSyncHistory = () => {
    ...
    sorted.sort((a, b) => {
      ...
      switch (syncSortColumn()) {
        case 'timestamp':
          aVal = new Date(a.syncTimestamp || a.startedAt || a.startTime || a.createdAt).getTime();
          bVal = new Date(b.syncTimestamp || b.startedAt || b.startTime || b.createdAt).getTime();
          break;
        case 'person':
        ...
        case 'type':
          ...
          break;
        case 'trigger':
          aVal = a.triggerType || 'MANUAL';
          bVal = b.triggerType || 'MANUAL';
          break;
        case 'status':
  ...
  const handleSyncTrigger = async () => {
    if (!confirm('Are you sure you want to trigger FULL SYNC? This will sync all data for all active persons.')) return;
    try {
      setLoading(true);
  ...
  const handleSyncMasterCandles = async () => {
    try {
      setLoading(true);
      showMessage('Syncing master market data (previous close for all symbols)...', 'info');
      await settingsApi.syncMasterCandles();
      showMessage('Master market data synced successfully - all symbols updated', 'success');
      await loadSyncStatus();
      await loadSyncHistory();
    } catch (error) {
      showMessage(`Master sync failed: ${error.message}`, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDailySyncAll = async () => {
    if (!confirm('Are you sure you want to trigger Phase 2 Daily Sync? This will sync core data (accounts, positions, balances) for all active persons.')) {
      return;
    }
    try {
      setLoading(true);
      showMessage('Starting Phase 2 Daily Sync (lightweight)...', 'info');
      await settingsApi.syncDailySyncAll();
      showMessage('Phase 2 Daily Sync completed successfully', 'success');
      await loadSyncStatus();
      await loadSyncHistory();
    } catch (error) {
      showMessage(`Phase 2 Daily Sync failed: ${error.message}`, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  createEffect(() => {
    ...
    // Dividend Manager has its own loadPositions() triggered by createEffect
  ...
  return (
    <div class="settings-container">
      ...
      <div class="settings-tabs">
        ...
        <button
          class={`settings-tab ${activeTab() === 'balances' ? 'active' : ''}`}
        ...
        <button
          class={`settings-tab ${activeTab() === 'apitest' ? 'active' : ''}`}
      ...
      <div class="settings-content">
          ...
          <div class="settings-section">
            ...
            <Show when={tokenInfo()}>
              <div class="token-info">
                ...
                <div class="info-row" style="margin-top: 16px; padding-top: 16px; border-top: 1px solid #333;">
                  <span class="info-label" style="font-weight: bold; color: #4a9eff;">Refresh Token Status:</span>
                  <span class="info-value mono-text" style={{color: tokenInfo().refreshTokenIsActive ? "#4caf50" : "#f44336"}}>{tokenInfo().refreshTokenIsActive ? "Active" : "Inactive"}</span>
                </div>
                <div class="info-row">
                  <span class="info-label">Refresh Token Expires:</span>
                  <span class="info-value mono-text">{formatDate(tokenInfo().refreshTokenExpiresAt)}</span>
                </div>
              </div>

            {/* Update Refresh Token */}
            <h3 class="section-subtitle" style="margin-top: 32px; margin-bottom: 12px; font-size: 14px; color: #888;">UPDATE REFRESH TOKEN</h3>
            <p style="margin-bottom: 16px; font-size: 13px; color: #999;">
              If your token has expired or is invalid, paste a new refresh token from Questrade here:
            </p>
            <div class="form-row">
              <input
                type="text"
                class="form-input"
                placeholder="Paste new refresh token from Questrade"
              ...
              <button
                ...
                disabled={loading() || !selectedPerson() || !newRefreshToken().trim()}
              >
                UPDATE TOKEN
              </button>
            </div>
          ...
          <div class="settings-section">
            ...
            <div class="sync-controls-grid">
              ...
              <button
                ...
                onClick={handleSyncMasterCandles}
                disabled={loading()}
                title="Sync master market data - updates previous close for ALL symbols in symbols-master table"
              ...
              <button
                ...
                disabled={loading()}
                title="Phase 2: Lightweight daily sync (accounts, positions, balances only)"
              ...
              <table class="settings-table">
                <thead>
                  <tr>
                    ...
                    <th class="sortable" onClick={() => handleSyncSort('trigger')}>
                      TRIGGER <span class="sort-icon">{getSyncSortIcon('trigger')}</span>
                    </th>
                ...
                <tbody>
                  <For each={sortedAndPaginatedSyncHistory()}>
                    {(sync) => (
                      <tr>
                        <td class="mono-text">{formatDate(sync.syncTimestamp || sync.startedAt || sync.startTime || sync.createdAt)}</td>
                        <td class="mono-text">{sync.personName || 'ALL'}</td>
                        <td class="mono-text">{(sync.syncType || sync.type || 'FULL').toUpperCase()}</td>
                        <td>
                          <span class={`status-badge ${sync.triggerType === 'SCHEDULED' ? 'info' : 'warning'}`}>
                            {sync.triggerType || 'MANUAL'}
                          </span>
                        </td>
        ...
        <Show when={activeTab() === 'dividends'}>
          <div class="settings-section">
            ...
            <DividendManager showMessage={showMessage} />
          </div>
        </Show>
        ...
        <Show when={activeTab() === 'balances'}>
          <div class="settings-section">
            <h2 class="section-title">ACCOUNT BALANCES &amp; CONTRIBUTIONS</h2>
            <AccountBalances showMessage={showMessage} />
          </div>
        </Show>
        ...
        {/* QUESTRADE API TEST TAB */}
        <Show when={activeTab() === 'apitest'}>
          <div class="settings-section">
            <h2 class="section-title">QUESTRADE API TEST</h2>
            <QuestradeApiTest persons={persons()} />
          </div>
        </Show>
      </div>
    </div>
