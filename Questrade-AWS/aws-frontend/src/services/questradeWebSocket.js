...
class QuestradeWebSocket {
  constructor() {
    ...
    this.scheduledReconnectInterval = null; // NEW: Scheduled reconnection every 25min
  ...
  /**
   * Get symbol IDs from backend (cached to avoid repeated API calls)
  ...
  async loadSymbolIds(symbols, append = false) {
    try {
      ...
      console.log('[QT WebSocket] Looking up symbols via backend (cached):', symbols);

      const response = await fetch(`${API_BASE_URL}/api/symbols/lookup`, {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(`Failed to lookup symbols: ${response.status}`);
      }
      ...
      // IMPORTANT: Convert symbolId to NUMBER to match WebSocket quote format
      Object.entries(data.data).forEach(([symbol, info]) => {
        if (info.symbolId) {
          const symbolIdNum = Number(info.symbolId); // Convert string to number
          this.subscribedSymbolIds.add(symbolIdNum);
          this.symbolIdMap.set(symbolIdNum, symbol);
        }
      });

      console.log('[QT WebSocket] Symbol ID mapping (numeric keys):', Object.fromEntries(this.symbolIdMap));
    } catch (error) {
  ...
  /**
   * Get stream port from backend (cached for 24 hours)
   */
  async getStreamPort(apiServer, accessToken, personName) {
    try {
      const API_BASE_URL = import.meta.env.VITE_API_GATEWAY_URL;

      console.log('[QT WebSocket] Getting stream port via backend (cached 24h)...');

      const symbolIdsArray = Array.from(this.subscribedSymbolIds);

      // Call backend endpoint to get stream port (backend caches for 24 hours)
      const response = await fetch(`${API_BASE_URL}/api/symbols/stream-port`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  handleMessage(message) {
    ...
    if (message.success === true) {
      ...
      // PHASE 1: Start scheduled reconnection every 25 minutes
      // This prevents Questrade's 30-minute timeout and token expiry issues
      this.startScheduledReconnection();
    ...
    if (message.quotes && Array.isArray(message.quotes) && message.quotes.length > 0) {
      ...
      console.log(`[QT WebSocket] [${timestamp}] 🔍 Processing ${message.quotes.length} quotes, symbolIdMap size: ${this.symbolIdMap.size}`);
      ...
      const processedQuotes = message.quotes
        ...
        .map(quote => {
          ...
          if (symbol) {
            ...
            console.warn(`[QT WebSocket] [${timestamp}] ⚠️ Symbol not found for symbolId: ${quote.symbolId}`);
          }
          return null;
        })
        .filter(quote => quote !== null);

      console.log(`[QT WebSocket] [${timestamp}] 📊 Processed ${processedQuotes.length} quotes with symbols`);
      ...
      } else if (!this.onQuoteUpdate) {
        console.error(`[QT WebSocket] [${timestamp}] ❌ onQuoteUpdate callback is NOT SET!`);
      } else if (processedQuotes.length === 0) {
        console.warn(`[QT WebSocket] [${timestamp}] ⚠️ No processed quotes (all symbols missing from map)`);
      }
  ...
  subscribeToSymbols() {
    ...
    // AGGRESSIVE FIX: Send multiple keepalive pings at short intervals
    // Questrade closes connection with code 1005 if no follow-up within 2-3 seconds
    // Send pings at 1s, 3s, 5s, and 10s to ensure connection stays alive
    const sendKeepalivePing = (delay, label) => {
      setTimeout(() => {
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
          console.log(`[QT WebSocket] 💓 Sending keepalive ping (${label})...`);
          const keepaliveMessage = {
            mode: 'streaming',
            ids: symbolIdsArray
          };
          this.ws.send(JSON.stringify(keepaliveMessage));
          console.log(`[QT WebSocket] ✅ Keepalive ping sent (${label})`);
        }
      }, delay);
    };

    // Send keepalive pings at multiple intervals
    sendKeepalivePing(1000, '1s after subscription');   // Very early ping
    sendKeepalivePing(3000, '3s after subscription');   // Second ping
    sendKeepalivePing(5000, '5s after subscription');   // Third ping
    sendKeepalivePing(10000, '10s after subscription'); // Fourth ping
  }
  ...
  /**
   * PHASE 1: Start scheduled reconnection every 25 minutes
   * Prevents Questrade's 30-minute timeout and token expiry
   */
  startScheduledReconnection() {
    // Clear any existing interval
    if (this.scheduledReconnectInterval) {
      clearInterval(this.scheduledReconnectInterval);
    }

    const RECONNECT_INTERVAL = 25 * 60 * 1000; // 25 minutes

    this.scheduledReconnectInterval = setInterval(() => {
      console.log('[QT WebSocket] ⏰ Scheduled reconnection (every 25 min to prevent token expiry)');
      console.log('[QT WebSocket] 🔄 Disconnecting and reconnecting with fresh token...');

      // Disconnect current connection
      if (this.ws) {
        this.ws.close(1000, 'Scheduled reconnection');
      }
      ...
      // Reconnect with fresh symbols and callback
      setTimeout(() => {
        console.log('[QT WebSocket] 🔌 Reconnecting after scheduled disconnect...');
        this.connect(Array.from(this.allSymbols), this.onQuoteUpdate);
      }, 2000); // Wait 2 seconds before reconnecting

    }, RECONNECT_INTERVAL);

    console.log('[QT WebSocket] ⏰ Scheduled reconnection enabled (every 25 minutes)');
  }
  ...
  cleanup() {
    ...
    if (this.scheduledReconnectInterval) {
      clearInterval(this.scheduledReconnectInterval);
      this.scheduledReconnectInterval = null;
    }
