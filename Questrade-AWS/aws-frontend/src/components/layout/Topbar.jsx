...
import { fetchCashBalances, fetchSyncStatus } from '../../services/api';
...
const Topbar = (props) => {
  ...
  const [selectedView, setSelectedView] = createSignal('all'); // 'all', 'person', 'account'
  const [selectedAccount, setSelectedAccount] = createSignal(null);

  // Per-login sync status (personName -> { needsReauth, lastSyncStatus, lastSyncTime, ... })
  const [syncStatus, setSyncStatus] = createSignal({});

  const loadSyncStatus = async () => {
    try {
      const list = await fetchSyncStatus();
      const map = {};
      (list || []).forEach((s) => { if (s && s.personName) map[s.personName] = s; });
      setSyncStatus(map);
    } catch (e) {
      console.warn('[TOPBAR] sync status load failed:', e.message);
    }
  };
  ...
  // Compute a {level: green|amber|red|unknown, label} for a login.
  const statusFor = (person) => {
    const s = syncStatus()[person];
    if (!s) return { level: 'unknown', label: '' };
    if (s.needsReauth) return { level: 'red', label: 'needs re-auth' };
    if (s.lastSyncStatus === 'failed') return { level: 'amber', label: 'last sync failed' };
    const t = s.lastSyncTime ? Number(s.lastSyncTime) : null;
    if (!t) return { level: 'amber', label: 'never synced' };
    const ageMs = Date.now() - t;
    const stale = ageMs > 24 * 60 * 60 * 1000;
    return { level: stale ? 'amber' : 'green', label: `synced ${relTime(ageMs)}` };
  };

  const anyNeedsReauth = () => Object.values(syncStatus()).some((s) => s && s.needsReauth);

  // Get persons from props
  const persons = () => {
    const personsList = props.persons && props.persons.length > 0 ? props.persons : ['Vivek'];
    // Remove "all" and "ALL" from persons dropdown (case-insensitive)
    return personsList.filter(p => p && p.toLowerCase() !== 'all');
  };
  ...
  const exchangeRate = () => props.exchangeRate || { rate: 1.40, percentChange: 0 };
  ...
  // Load accounts on mount - fetch ALL accounts from ALL persons
  onMount(async () => {
    loadSyncStatus(); // per-login status dots

    try {
      // Fetch accounts for 'all' to get all accounts from all persons
      const cashData = await fetchCashBalances('all');
      if (cashData && cashData.accounts) {
        setAccounts(cashData.accounts);
        console.log('[TOPBAR] Loaded', cashData.accounts.length, 'accounts from all persons');
      }
    } catch (error) {
  ...
  // NOTE: Removed createEffect that reloaded accounts when person changed
  // Dropdown should always show ALL accounts from ALL persons

  // Refresh per-login status right after a sync finishes (isSyncing true -> false).
  let prevSyncing = props.isSyncing;
  createEffect(() => {
    const now = props.isSyncing;
    if (prevSyncing && !now) loadSyncStatus();
    prevSyncing = now;
  });
    ...
    <div class="topbar">
      ...
      <div class="topbar-center">
        ...
        <div class="topbar-dropdown" onClick={() => setShowAccountDropdown(!showAccountDropdown())}>
          ...
          <Show when={anyNeedsReauth()}>
            <span class="reauth-warning" title="A login needs re-authorization">⚠</span>
          </Show>
                  ...
                  {(person) => (
                    ...
                    >
                      ...
                      <div class="option-content">
                        ...
                        <span class={`sync-status sync-${statusFor(person).level}`}>
                          <span class="sync-dot"></span>
                          {statusFor(person).label}
                        </span>
                      </div>
                    </div>
                  )}
        ...
        <div class="topbar-chip rate-chip">
          <span class="chip-text">USD/CAD: {exchangeRate().rate.toFixed(4)}</span>
          <span
            class="chip-percent"
            style={{
              color: exchangeRate().percentChange >= 0 ? '#10b981' : '#ef4444',
              'margin-left': '8px'
            }}
          >
            {exchangeRate().percentChange >= 0 ? '+' : ''}{exchangeRate().percentChange.toFixed(2)}%
          </span>
        </div>
      </div>
