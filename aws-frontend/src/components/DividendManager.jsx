...
const DividendManager = (props) => {
  ...
  // Sync dividends from Questrade API
  const handleSyncDividends = async () => {
    try {
      ...
      // Reload positions to show updated data
      await loadPositions();
    } catch (error) {
  ...
  const loadPositions = async () => {
    try {
      ...
      console.log('[DIVIDEND] Manual overrides found:', overrides.length, overrides);
  ...
  const getOverrideValue = (symbol) => {
    ...
    // Note: isManualOverride comes as string "true"/"false" from DynamoDB
    const isManualOverride = symbolData?.isManualOverride === 'true' || symbolData?.isManualOverride === true;
    if (isManualOverride && symbolData?.monthlyDividendPerShare) {
  ...
  const handleSaveAll = async () => {
    try {
      ...
      // Clear changes immediately to prevent UI flicker
      setChanges({});
      ...
      for (const [symbol, changeTypes] of Object.entries(symbolChanges)) {
        ...
        if ('include' in changeTypes) {
          if (changeTypes.include) {
            ...
            await settingsApi.removeYieldExclusion(symbol); // GLOBAL - applies to all users
          } else {
            // Exclude from YoC
            await settingsApi.addYieldExclusion(symbol, 'User excluded'); // GLOBAL - applies to all users
          }
        }
      ...
      // Wait for backend to fully process writes (DynamoDB eventual consistency)
      await new Promise(resolve => setTimeout(resolve, 1000));

      // Reload all data from database
      await loadPositions();

      console.log('[DIVIDEND] Reload complete after save, symbolDividends:', symbolDividends());
    } catch (error) {
      props.showMessage?.(`Failed to save changes: ${error.message}`, 'error');
      // Restore changes on error so user can retry
      setChanges(Object.fromEntries(changeEntries));
    } finally {
      setLoading(false);
    }
  };
  ...
  // Copy symbol to clipboard (with fallback for HTTP)
            ...
            <Show when={filteredPositions().length > 0} fallback={
              ...
              <For each={filteredPositions()}>
                {(position) => {
                  ...
                  // Don't store overrideValue or frequencyValue in const - call functions directly in JSX for reactivity
                      ...
                      <td>
                        <div class="override-cell">
                          <input
                            ...
                            value={getOverrideValue(position.symbol)}
                            onChange={(e) => handleOverrideChange(position.symbol, e.target.value)}
                            placeholder="0.00"
                          />
                          <Show when={getOverrideValue(position.symbol) !== ''}>
