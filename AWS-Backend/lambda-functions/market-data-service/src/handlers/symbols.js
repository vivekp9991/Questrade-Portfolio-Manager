...
// Helper to get access token from tokens table
async function getAccessToken(personName) {
  // Query tokens table for access token
  ...
  if (result.items.length === 0) {
    throw new Error(`No access token found for ${personName}`);
  }
  ...
  if (token.expiresAt && new Date(token.expiresAt) <= new Date()) {
    throw new Error(`Access token expired for ${personName}`);
  }

  // Decrypt token
  const accessToken = decrypt(token.encryptedToken);
...
// Helper to call Questrade API
async function callQuestradeApi(personName, endpoint) {
  try {
    const tokenData = await getAccessToken(personName);

    // Ensure apiServer has trailing slash (Questrade note: apiServer already has trailing /)
    const baseUrl = tokenData.apiServer.endsWith('/') ? tokenData.apiServer : `${tokenData.apiServer}/`;

    // Build URL - endpoint should NOT start with / to avoid double slash
