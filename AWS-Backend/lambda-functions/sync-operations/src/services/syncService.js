...
class SyncService {
   ...
   */
  async getAccountsWithCache(personName) {
    ...
    logger.info(`[PHASE 5] Cache miss - fetching accounts from API for ${personName}`);
  ...
  async getPerson(personName) {
    try {
      const result = await query(PERSONS_TABLE, {
  ...
  async syncPerson(personName, syncType = 'full') {
    ...
    try {
      ...
      if (syncType === 'full' || syncType === 'accounts') {
        // PHASE 5: Use cached accounts
        const accounts = await this.getAccountsWithCache(personName);
        await this.syncAccounts(personName, accounts);
  ...
  async syncAllPersons() {
    try {
      ...
      const result = await scan(PERSONS_TABLE, {
        FilterExpression: 'isActive = :isActive AND hasValidToken = :hasValidToken',
   ...
   */
  async syncAccounts(personName, accounts) {
    const items = [];
  ...
  async fetchActivitiesOptimized(personName, accountNumber) {
    ...
    if (!person || !person.activitiesInitialized) {
      // First-time setup: Fetch 5 years of historical data
      logger.info(`[PHASE 3] First-time activities sync for ${personName} - Fetching 5 years of history`);
      return await activitySyncHelper.fetchActivitiesHistorical(personName, accountNumber, 5);
    } else {
  ...
  async syncActivities(personName, accountNumber, activities) {
    const items = activities
      ...
      .map((activity, index) => {
        // Create unique activityDateTime by appending index to avoid duplicates
        // If multiple activities have same transactionDate, append index for uniqueness
        const activityDateTime = `${activity.transactionDate}_${index}_${activity.symbolId || 'nosymbol'}`;
   ...
   */
  async getSyncStatus() {
    // Get last sync for each person
    const result = await scan(SYNC_HISTORY_TABLE);

    // Group by person and get latest
    const statusByPerson = {};
    result.items.forEach(item => {
      if (!statusByPerson[item.personName] || item.syncTimestamp > statusByPerson[item.personName].syncTimestamp) {
        statusByPerson[item.personName] = item;
      }
    });

    return Object.values(statusByPerson);
  }
